...
type Relation = "spouse" | "parent" | "sibling" | "child" | "friend" | "guardian"

const RELATIONS: Relation[] = ["spouse", "parent", "sibling", "child", "friend", "guardian"]
...
export default function EmergencyContactsPage() {
  ...
  const handleAdd = async () => {
    if (!form.contact_wallet || !form.name) {
    ...
    try {
      ...
      toast.success("Contact added", {
        description: `${form.name} can now view your reports.`
      })
    ...
    <div className="space-y-6 max-w-2xl">
      <div>
        ...
        <p className="text-sm text-muted-foreground mt-1">
          Up to 3 contacts — same access level as a doctor
        </p>
      </div>
      ...
      <Card>
        <CardHeader>
          <CardTitle className="text-sm font-medium">Add emergency contact</CardTitle>
        </CardHeader>
        ...
        <CardContent className="space-y-4">
          ...
          <div className="space-y-1.5">
            ...
            <Input
              placeholder="0x..."
              value={form.contact_wallet}
              onChange={e =>
                setForm(f => ({ ...f, contact_wallet: e.target.value }))
              }
            />
          </div>
          ...
          {contacts.length === 0 ? (
            <div className="text-center py-8 space-y-2">
              ...
              <p className="text-sm text-muted-foreground">
                No contacts. Click refresh to load from chain.
              </p>
            </div>
          ) : (

...
const REPORT_TYPES = ["lab_report", "prescription", "xray", "scan"]
...
export default function ReportsPage() {
  ...
  const handleUpload = async () => {
    ...
    try {
      ...
      setUploadResult(res)
      toast.success("Report uploaded", { description: "Encrypted on IPFS + stored on blockchain." })
    } catch (e: any) {
  ...
  return (
    <div className="space-y-6 max-w-3xl">
      <div>
        ...
        <p className="text-sm text-muted-foreground mt-1">Upload and view encrypted healthcare reports</p>
      </div>

      <Card>
        <CardHeader><CardTitle className="text-sm font-medium">Upload report</CardTitle></CardHeader>
        <CardContent className="space-y-4">
          <div className="grid grid-cols-2 gap-4">
            ...
            <div className="space-y-1.5">
              ...
              <Select value={reportType} onValueChange={setReportType}>
                ...
                <SelectContent>
                  {REPORT_TYPES.map(t => <SelectItem key={t} value={t}>{t.replace("_", " ")}</SelectItem>)}
                </SelectContent>
              </Select>
            </div>
          </div>
          ...
          >
            ...
            <p className="text-sm text-muted-foreground">{file ? file.name : "Click to select PDF or image"}</p>
            <p className="text-xs text-muted-foreground mt-1">Encrypted with AES-256-GCM before upload</p>
            <input id="file-input" type="file" className="hidden" accept=".pdf,image/*"
              onChange={e => setFile(e.target.files?.[0] || null)} />
          </div>

          <Button onClick={handleUpload} disabled={uploading} className="w-full bg-teal-600 hover:bg-teal-700">
            {uploading
              ? <><Loader2 className="w-4 h-4 mr-2 animate-spin" />Encrypting + uploading...</>
              : <><Upload className="w-4 h-4 mr-2" />Encrypt + Upload</>}
          </Button>
      ...
      <Card>
        <CardHeader className="flex flex-row items-center justify-between">
          <CardTitle className="text-sm font-medium">Stored reports · {patientId}</CardTitle>
                ...
                <div key={i} className="flex items-center justify-between p-3 rounded-lg bg-muted/50 border border-border">
                  <div className="space-y-0.5">
                    <div className="flex items-center gap-2">
                      ...
                      <Badge variant="outline" className="text-xs">{r.is_active ? "active" : "inactive"}</Badge>
                    </div>
                  ...
                  <Button size="sm" variant="outline" onClick={() => handleDownload(r.ipfs_cid, r.nonce, r.report_type)}>
                    <Download className="w-3 h-3 mr-1" />Decrypt
                  </Button>
                </div>
              ))}
            </div>

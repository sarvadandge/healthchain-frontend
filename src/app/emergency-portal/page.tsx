...
import {
  ...
  FileText, ShieldCheck, ShieldX, RefreshCw, Phone,
} from "lucide-react"
...
export default function EmergencyPortalPage() {
    ...
    try {
      ...
      if (roleRes.role === "none") {
        toast.error("Access denied", {
          description: `You are not listed as an emergency contact for patient ${patientId}.`,
        })
        return
      }

      if (roleRes.role === "doctor" || roleRes.role === "owner") {
        toast.info("Use the Doctor Portal", {
          description: "Your role is not emergency contact. Use /doctor instead.",
        })
      }
  ...
  return (
    <div className="space-y-6 max-w-3xl">
      <div>
        <h1 className="text-2xl font-semibold tracking-tight">Emergency Portal</h1>
        <p className="text-sm text-muted-foreground mt-1">
          View patient data if you are listed as an emergency contact
        </p>
      </div>
      ...
      {!isConnected && (
        <div className="rounded-lg border border-amber-200 bg-amber-50 dark:bg-amber-950 px-4 py-3">
          <p className="text-sm text-amber-800 dark:text-amber-200">
            Connect your MetaMask wallet — your wallet address is checked on-chain for emergency access.
          </p>
        </div>
      )}

      <div className="rounded-lg border border-orange-200 bg-orange-50 dark:bg-orange-950 px-4 py-3">
        <div className="flex items-center gap-2">
          <Phone className="w-4 h-4 text-orange-600 shrink-0" />
          <p className="text-sm text-orange-800 dark:text-orange-200">
            This portal is for emergency contacts only. You must be added by the patient to access their data.
          </p>
        </div>
      </div>
      ...
      {/* Search */}
      <Card>
        <CardHeader><CardTitle className="text-sm font-medium">Find patient</CardTitle></CardHeader>
